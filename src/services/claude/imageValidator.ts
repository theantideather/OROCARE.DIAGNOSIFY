...
export class ImageValidator {
  async validate(imageBase64: string): Promise<void> {
    ...
    if (sizeInBytes > CLAUDE_CONFIG.MAX_IMAGE_SIZE) {
      throw new Error(CLAUDE_CONFIG.ERROR_MESSAGES.IMAGE_TOO_LARGE);
    }

    // Check image format from data URL
    if (imageBase64.includes('data:')) {
      const format = this.getImageFormat(imageBase64);
      if (!CLAUDE_CONFIG.SUPPORTED_IMAGE_FORMATS.includes(format)) {
        throw new Error(CLAUDE_CONFIG.ERROR_MESSAGES.UNSUPPORTED_FORMAT);
      }
    }
  }
